...
- [ ] 본 Spec(버그 수정)이 *건드리는* 영역만 골랐는가
- [ ] 버그가 invariant 위반에서 비롯되었다면 그 invariant가 정의된 섹션을 포함했는가 (예: 권한 누수 → `Permissions & Role Model`)

**Pushback probes:**

- "버그가 invariant 위반인지 vs 단순 구현 결함인지 한 번 구분해보세요. invariant 위반이면 Steering 섹션 명시가 필수."
