...
1. **Notion MCP** must be connected (check by listing available tools). If not connected, tell the user:

   > "먼저 Notion MCP를 연결해주세요. 연결 없이는 트리거를 저장할 수 없습니다."
   > and stop.

2. The following Notion DB IDs are fixed for this workspace:
...
3. Templates in Notion are NOT used by this skill. All content is generated by the skill and written directly via `mcp__notion__notion-create-pages`.
...
- Use `mcp__notion__notion-search` with `data_source_url: "collection://33ecd014-3f6e-80c0-b177-000b3e08ac88"` and user's keyword.
- Show up to 5 results with title + 유형 + 생성일. Let the user pick by number.
- Fetch the selected Request's body content via `mcp__notion__notion-fetch` and summarize.
- Save the Request page URL for later (will be set as the `Requests DB` relation property).
...
mcp__notion__notion-search(
  query = "", # or keyword if user gave hint
  data_source_url = "collection://d47fc205-0235-4dc6-b156-843e80a928e7"
)
```

Show a numbered list of non-archived projects. Let user pick by number. Save the Project page URL.
...
```
parent: { type: "data_source_id", data_source_id: "33fcd014-3f6e-8092-a585-000b407693e7" }
pages: [
  {
    properties: {
      "이름": "<generated or user-supplied title>",
      "출처": "<Strategic | Customer | Ops | Product>",
      "허용 기간": "<1 Week | 2 Weeks | 6 Weeks | 3 Months | 6+ Months>",
      "우선순위": "<P0 | P1 | P2 | P3>",
      "Projects DB": "<project page URL from Phase 0.2>",
      "Requests DB": "<request page URL from Phase 0.1 or omit>"
    },
    content: "<full markdown body of 7 sections, see 8.4>"
  }
]
```

Do **not** set `상태` at creation. Leave it empty (active). Only set `상태` to `Success` or `Killed` when the Initiative terminates.
...
- **Notion MCP not connected** → stop at Prerequisites check, tell user to connect MCP.
