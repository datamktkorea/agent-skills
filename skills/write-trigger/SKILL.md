---
name: write-trigger
description: 부키 개발 프로세스의 첫 단계인 트리거 문서를 작성하는 스킬. 사용자가 개발을 시작하게 된 상황을 설명하면, 트리거 유형(Fix/Build/Improve/Strategy)과 우선순위(P0–P3)를 함께 결정하고, Problem Statement를 올바른 형식으로 완성한 뒤 트리거 문서를 생성한다. "트리거 문서 써줘", "개발 시작 전에 정리해야 해", "왜 이 기능 만드는지 문서화해야 해", "트리거 작성" 등의 요청에 반드시 이 스킬을 사용한다.
---
